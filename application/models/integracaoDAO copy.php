...
class integracaoDAO extends CI_Model {
	...
	{
		$sql = "SELECT top 1 * FROM cliente WHERE cpfcnpj = ?";

		$query = $this->db->query($sql, array($cpfCli));

		if ($query->num_rows() > 0) {
			// Cliente encontrado, retorna os detalhes do cliente
	...
	public function setCliente($nomeCli,$cpfFormatado,$logradouro,$cep,$numero,$complemento,$bairro,$cidade,$uf,$telefone,$email) 
	{	

		// $cpfFormatado = substr($cpfCli, 0, 3) . '.' . substr($cpfCli, 3, 3) . '.' . substr($cpfCli, 6, 3) . '-' . substr($cpfCli, 9, 2);

		$sql = "INSERT INTO cliente (nome,jurRazaoSocial,cpfcnpj,endLogradouro,endNumero,endBairro,endCidade,endCep,endComplemento,endEstado,endFone,email,datainclusao) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,'2024-04-05')";
